...
var (
	...
	targetDirPtr = flag.String(PARAMETER_TARGET, jira_core.TARGET, "target")
)
...
func do(
	...
	sourceDir := fmt.Sprintf("atlassian-jira-core-%s-standalone", extractAtlassianVersion(config.Version))
...
func createDefaultConfig() *debian_config.Config {
	...
	config.Name = jira_core.PACKAGE_NAME
	config.Architecture = jira_core.ARCH
	return config
}
