...
import (
	...
	debian_latest_package_creator "github.com/seibert-media/debian-utils/latest_package_creator"
...
var (
	...
	targetDirPtr = flag.String(PARAMETER_TARGET, bitbucket.TARGET, "target")
)
...
func do(
	...
	sourceDir := fmt.Sprintf("atlassian-bitbucket-%s", config.Version)
...
func createDefaultConfig() *debian_config.Config {
	...
	config.Name = bitbucket.PACKAGE_NAME
	config.Architecture = bitbucket.ARCH
	return config
}
